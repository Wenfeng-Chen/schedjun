...
class ScheduleUpdateIntegrationTest {
    ...
    @Test
    void updateScheduleReturnsSuccess() throws Exception {
        String token = loginAndGetToken();
        String scheduleId = createScheduleAndGetId(token);

        CreateScheduleDTO dto = buildScheduleDto();
        dto.setId(scheduleId);
        dto.setTitle("更新后的日程标题");
        dto.setNotes("更新后的备注");
        ...
        assertEquals(scheduleId, data.path("id").asText());
        assertEquals("更新后的日程标题", data.path("title").asText());
        assertEquals("更新后的备注", data.path("notes").asText());
    }
