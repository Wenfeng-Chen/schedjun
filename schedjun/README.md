...
参考 [doc/db.md](./doc/db.md) 执行建表 SQL，至少包含 `user`、`schedule`、`assistant_message` 三张表。
...
```bash
cd backend
mvn test -Dtest=AssistantVoiceIntegrationTest,AssistantConfirmIntegrationTest,AssistantIntentResolverTest,ChineseTimeParserTest
```
